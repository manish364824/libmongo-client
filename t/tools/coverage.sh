...
for src in ${SOURCES}; do
	case "$src" in
		*.c)
			...
			if test -f "${builddir}/.libs/$obj"; then
				objdir=${builddir}/.libs
			else
				objdir=${builddir}
			fi
			if ! test -f "${objdir}/${gc}"; then
				continue
			fi
			gcov -b -f ${srcdir}/$src -o $objdir/$obj >coverage/$src.cov
			;;
	esac
done

perl ${top_srcdir}/t/tools/coverage-report.pl coverage/*.cov >coverage/index.xml
xsltproc ${top_srcdir}/t/tools/coverage-report.xsl coverage/index.xml >coverage/index.html
